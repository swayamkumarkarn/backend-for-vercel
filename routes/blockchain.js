...
/**
 * 
 * Returns the Lastest list of transaction for a specific public Key
 * 
 */
router.get('/latest/transactions/:key', async (req, res) => {
    const result = blockchain.getLatestTransactionForWallet(req.params.key);
    res.status(200).json(result);
});
...
router.post('/transactions', async (req, res) => {
    try {
        ...
        const fee = new Transaction(req.body.from, req.body.private, "9999999999", parseInt(req.body.amount / 100), "Transaction Fee");
